 ...
 */
void conv(const T kernel[k][k], const T image[M + (k - 1)][N + (k - 1)],
          T imagecamb[M + (k - 1)][N + (k - 1)]) {
  int start = k / 2;
  T suma = 0;
    ...
    for (int j = start; j < N + start; j++) {
      for (int e = 0; e < k; e++) {
        for (int r = 0; r < k; r++) {
          ...
          // if(i == 2 && j == 2){
          //	std::cout<< "Posicion :" << "(" << e << "  , " << r << ")" <<
          //"(" << i-start+e << "  , " << j-start+r << ")" << std::endl <<
          //kernel[e][r] << " * " << image[i-start+e][j-start+r] << " = " <<
          //suma << std::endl;
          //}
        }
      }
